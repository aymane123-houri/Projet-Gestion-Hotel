...
public class AuthenticationController {
    ...
    public Map<String, String> login(@RequestBody Credentials credentials){
        return generateJWTService.generateToken(credentials.getEmail(), credentials.getPassword());
    }
    @PostMapping("/refreshToken")
    public Map<String, String> refresh(@RequestParam String email, @RequestParam String password){
        return generateJWTService.generateToken(email, password);
    }
